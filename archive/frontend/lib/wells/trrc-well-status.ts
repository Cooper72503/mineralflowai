/**
 * TRRC Well Status Query (EWA)
 *
 * Source: TRRC EWA wellStatusQueryAction.do
 *   https://webapps2.rrc.texas.gov/EWA/wellStatusQueryAction.do
 *
 * Returns current well operating status (Active, Inactive, Shut-in, Plugged),
 * well type, on-schedule indicator, and formation name for a specific API.
 *
 * Two query modes:
 *
 ...
 *   6: Operator Name
 *   7: County
 *   8: Well Type        (e.g. "Oil Well", "Gas Well", "Water Supply")
 *   9: Current Status   (e.g. "Active", "Inactive", "Shut-in", "Plugged")
 *  10: On Schedule      (Y/N)
...
export type TrrcWellStatusRecord = {
...
/**
 * Parse EWA well status results HTML.
 *
 ...
 */
function parseWellStatusHtml(html: string): TrrcWellStatusRecord[] {
  const results: TrrcWellStatusRecord[] = [];
  ...
  while ((apiMatch = apiLinkPattern.exec(html)) !== null) {
    ...
    const onScheduleRaw = (vals[10] ?? "").toUpperCase().trim();
    const on_schedule: boolean | null =
      onScheduleRaw === "Y" ? true :
      onScheduleRaw === "N" ? false : null;
    ...
    results.push({
      ...
      county:         vals[7] || null,
      well_type:      vals[8] || null,
      current_status: vals[9] || null,
      on_schedule,
      status_date:    vals[11] || null,
      formation_name: vals[12] || null,
    });
  }
...
async function _fetchTrrcWellStatus(
  api10:  string,
  signal: AbortSignal,
): Promise<TrrcWellStatusRecord[]> {
  ...
  const res = await fetch(`${EWA_BASE}/wellStatusQueryAction.do`, {
    method: "POST",
    headers: {
  ...
  return parseWellStatusHtml(html);
}

async function _fetchTrrcWellStatusByLease(
  district: string,
  leaseNo:  string,
  signal:   AbortSignal,
): Promise<TrrcWellStatusRecord[]> {
...
/**
 * Fetch well status record(s) for a well by API number.
 * Returns [] when no record exists or on failure. Never throws.
 */
export async function fetchTrrcWellStatus(api10: string): Promise<TrrcWellStatusRecord[]> {
  if (!api10?.trim()) return [];
  return withTrrcRetry(sig => _fetchTrrcWellStatus(api10, sig), [], { timeout: 15_000 });
}

/**
 * Fetch well status records for a lease by district + lease number.
 * Returns [] on failure. Never throws.
 */
export async function fetchTrrcWellStatusByLease(
  district: string,
  leaseNo:  string,
): Promise<TrrcWellStatusRecord[]> {
  if (!leaseNo?.trim()) return [];
  return withTrrcRetry(
    sig => _fetchTrrcWellStatusByLease(district, leaseNo, sig),
    [],
    { timeout: 15_000 },
  );
}
