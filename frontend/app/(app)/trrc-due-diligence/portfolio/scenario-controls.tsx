...
export function ScenarioControls({onChange}:{onChange:Dispatch<SetStateAction<Record<string,unknown>|null|undefined>>}){
 ...
 return <div style={{margin:"1rem 0"}}><label><input type="checkbox" checked={enabled} onChange={e=>setEnabled(e.target.checked)}/> Calculate a conditional oil-only acquisition and exit scenario</label>
 ...
 <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(220px,1fr))",gap:"0.7rem"}}>{fields.map(([key,label])=><label key={key} style={{fontSize:"0.8rem"}}>{label}<input type="number" min="0" step="any" value={values[key]} onChange={e=>setValues(v=>({...v,[key]:e.target.value}))} style={{width:"100%",background:COLORS.surfaceAlt,color:COLORS.text,border:`1px solid ${COLORS.border}`,padding:"0.4rem"}}/></label>)}</div></>}
 </div>;
}
