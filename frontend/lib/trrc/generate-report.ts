/**
 * Shared "load a completed run and generate its PDF report" logic —
 * extracted from GET /api/trrc/due-diligence/[runId]/report so the bulk
 * report-bundle endpoint can generate N reports without duplicating the
 * full data-loading/manifest-assembly sequence. Pure refactor: the
 * single-run route now calls this same function.
 */
...
export type GenerateReportResult =
  | { ok: true; pdfBuffer: Buffer; filename: string; run: TrrcDueDiligenceRun }
  | { ok: false; error: string; status: number };

export async function generatePdfReportForRun(
  ...
  if (runError || !runRaw) {
    return { ok: false, error: "Run not found or access denied.", status: 404 };
  }

  if (runRaw["status"] !== "complete") {
    return { ok: false, error: "Report is only available for completed runs.", status: 409 };
  }
  ...
  const manifest: TrrcManifest = {
    ...
    source_attempts: sourceAttemptRows as unknown as import("./types").SourceAttempt[],
    discovered_records: [],
    downloaded_files: [],
    failed_downloads: [],
  ...
  try {
    const mod = await import("./report-builder");
  ...
  try {
    ...
    console.error(`[generatePdfReportForRun] PDF generation error for ${runId}:`, msg);
    return { ok: false, error: "Failed to generate PDF report.", status: 500 };
  }
  ...
  return { ok: true, pdfBuffer, filename, run };
}
