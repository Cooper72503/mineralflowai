 ...
 *
 * Copied directly from geology/evidence.ts (same shape, same guardrail) —
 * see that file's header for the original rationale.
 */

import { randomUUID } from "crypto";
import type { TitleEvidenceEntry, StatementClassification } from "./types";
...
export function recordEvidence(input: RecordEvidenceInput): TitleEvidenceEntry {
  return {
    id: randomUUID(),
...
/** Calculated values must preserve the inputs and formula used — this is the one helper that enforces a non-empty transformationMethod for classification="calculated", since a calculated value with no stated formula is exactly the "hidden math" this engine must never produce. */
export function recordCalculatedEvidence(input: Omit<RecordEvidenceInput, "classification"> & { transformationMethod: string }): TitleEvidenceEntry {
