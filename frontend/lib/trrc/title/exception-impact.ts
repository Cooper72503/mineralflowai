...
const PROFILES: Partial<Record<ChainFindingType, Profile>> = {
  FRACTION_INCONSISTENCY: {
    affects: ["ownership", "nri", "value"], affectsNote: "Conflicting stated fractions prevent reliance on the ownership calculation.",
    blocksClosing: true, decisionMaterial: true, quantifiable: false, ownershipMaterial: true, requiresProfessionalReview: true,
    resolution: "Reconcile the cited fractions and review the controlling instrument before relying on NRI.",
    unquantifiedReason: "The controlling fraction has not been established; no allocation is assumed.",
  },
  TRACT_INTEREST_MISMATCH: {
    affects: ["ownership", "coverage"], affectsNote: "The instrument's tract or interest scope does not match the evaluated position.",
    blocksClosing: true, decisionMaterial: true, quantifiable: false, ownershipMaterial: true, requiresProfessionalReview: true,
    resolution: "Confirm the instrument's legal tract and interest scope against the evaluated position.",
    unquantifiedReason: "An unresolved tract or interest mismatch cannot be converted into a dollar exposure.",
  },
