/**
 * Texas county name -> RRC's 3-digit county code, used as the `countyNames`
 * value in the W-1 public query POST body. These match the standard Texas
 * county FIPS codes (odd numbers 001-507) and were read directly off the
 * live query form's <select name="countyNames"> options at
 * https://webapps.rrc.state.tx.us/DP/initializePublicQueryAction.do — not
 * guessed.
 */
export const COUNTY_CODES: Record<string, string> = {
  ...
  DENTON: "121", "DE WITT": "123", DICKENS: "125", DIMMIT: "127", DONLEY: "129",
...
export function countyCodeFor(countyName: string): string | null {
  return COUNTY_CODES[countyName.trim().toUpperCase()] ?? null;
}

export const ALL_COUNTY_NAMES: string[] = Object.keys(COUNTY_CODES).sort();
