...
export async function loadPortfolioInputs(db:SupabaseClient,userId:string,raw:unknown){
 const input=PortfolioRequest.parse(raw);
 const ids=[...new Set(input.members.flatMap(m=>m.runId?[m.runId]:[]))];
 const runs:(RetainedRun & {updated_at:string;title_research_job_id?:string|null})[]=[];
 ...
 for(let start=0;start<ids.length;start+=4){
  const batch=await Promise.all(ids.slice(start,start+4).map(async id=>{
   const {data:run,error}=await db.from("trrc_due_diligence_runs").select("id,original_input,status,resolved_primary_api,updated_at,title_research_job_id").eq("id",id).eq("user_id",userId).maybeSingle();
