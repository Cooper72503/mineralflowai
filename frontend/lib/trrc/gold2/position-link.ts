...
export async function loadReviewedPosition(db:SupabaseClient,userId:string,apiInput:string,title:TitleChainAnalysis|null){
 ...
 if(job.error||!job.data)throw Error("Reviewed position selection could not be loaded");
 if(job.data.latest_analysis_id!==title.analysisId)throw Error("Title analysis changed during position loading");
 ...
 if(saved.error||!saved.data)throw Error("Selected reviewed position could not be loaded");
