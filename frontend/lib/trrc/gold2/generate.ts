...
export async function generateGold2ForRun(db:SupabaseClient,runId:string,userId:string,format:"pdf"|"json",supplementInput:unknown={}){
 ...
 const api=normalizeApiNumber(run.original_input)?.api10??normalizeApiNumber(run.resolved_primary_api)?.api10;
 if(api&&run.resolved_primary_api&&normalizeApiNumber(run.resolved_primary_api)?.api10!==api)return {ok:false as const,status:422,error:"Resolved API conflicts with the requested API; report withheld."};
